<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <v-data-table :headers="headers" :items="subjects" class="elevation-1" style="width: 80%;">
    <template v-slot:top>
      <v-toolbar flat color="white">
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }">
            <v-btn color="primary" dark class="mb-2" v-on="on" outlined>Nova disciplina</v-btn>
          </template>
          <v-card>
            <v-card-title>
    ...
    <template v-slot:no-data>
      <p class="title mt-2 mb-2">Nenhuma disciplina disponível</p>
    </template>
...
<script>
  export default {
    ...
    computed: {
      formTitle() {
        return this.editedIndex === -1 ? 'Nova Disciplina' : 'Editar Disciplina'
      }
    },
    watch: {
      dialog(val) {
    ...
    methods: {
      ...
      deleteItem(item) {
        ...
        confirm('Deseja mesmo remover esse professor?') && this.subjects.splice(index, 1)
      },
